...
def plot_lp_wern(dimension):
    ...
    for q in q_vals:
        constraints = [r1 >= 0, r3 >= 0]
        constraints += [r1 + d * r2 >= 0, r3 + d * r4 >= 0]
        constraints += [(d * (r1 + r3) + r2 + r4) <= mu]
        constraints += [r1 + r2 - r3 - r4 + q - q / d - 1 >= 0]
        constraints += [r1 + r4 - r2 - r3 - q / d + 1 - q >= 0]
        prob = cp.Problem(cp.Minimize(mu), constraints)
        prob.solve()
        c.append(log2(prob.value))
    ...
    plt.xlabel('q-parameter')
    plt.ylabel('quantum capacity')
    plt.savefig('./plot/werner/lp_wern_' + str(d) + '.png')
...
def plot_all_lp_depol(upper):
    ...
    for i in (listt):
        ...
        plt.title('Depolarizing Channel with d from ' + str("2") + ' to ' + str(upper))
        # plt.xlabel('q-parameter')
        plt.xlabel('t-parameter')
        plt.ylabel("log" + r'$\Gamma(N)$')
        plt.savefig('./plot/depol/lp_depol_' + str("2") + '_' + str(upper) + '.png')
...
def plot_all_lp_wern(upper):
    ...
    for i in listt:
        ...
        plt.title('Werner-Holevo Channel with d from ' + str("2") + ' to ' + str(upper))
        # plt.xlabel('q-parameter')
        plt.xlabel('t-parameter')
        plt.ylabel("log" + r'$\Gamma(N)$')
        plt.savefig('./plot/werner/lp_wern_' + str("2") + '_' + str(upper) + '.png')
